...
const referenceSource = (entityName, source) => `${entityName}@${source}`;
const noFilter = () => true;

/**
 * An Input component for choosing a reference record. Useful for foreign keys.
 *
 * This component fetches the possible values in the reference entityName
...
export class ReferenceInput extends Component {
    ...
    fetchReferenceAndOptions({ input, reference, source, entityName } = this.props) {
        const { pagination, sort, filter } = this.params;
        const id = input.value;
        if (id) {
            this.props.crudGetOne(reference, id, null, false);
        }
        this.props.crudGetMatching(reference, referenceSource(entityName, source), pagination, sort, filter);
    }

    render() {
        const { input, entityName, label, source, referenceRecord, allowEmpty, matchingReferences, basePath, onChange, children } = this.props;
